...
def main():
    ...
    sft_config = SFTConfig(
        ...
        max_length=max_seq_length,         # 2500 (trl 최신버전: max_seq_length → max_length)
        ...
        logging_steps=25,
        save_total_limit=1,               # 체크포인트를 최대 1개만 유지 (이전 것 자동 삭제)
        overwrite_output_dir=True,        # output_dir 재실행 시 덮어쓰기 (누적 방지)
    )
