...
def main():
    ...
    # dtype="auto"는 모델 config 기준으로 bf16을 선택할 수 있어 fp16 스케일러와 충돌함
    # GTX 1650(Turing)은 bf16 미지원이므로 명시적으로 float16으로 강제 지정
    ...
    print(f"[INFO] 모델 dtype: {model_dtype}")
    ...
    # -------------------------------------------------------------------
    # GTX 1650 (VRAM 4GB) 환경에 맞춰 조정된 값
    # max_seq_length: 2500 → 512 (VRAM 사용량을 가장 크게 줄이는 핵심 파라미터)
    # overlap_size:   800  → 128 (max_seq_length에 비례해 축소)
    max_seq_length = 512
    ...
    training_args = TrainingArguments(
        ...
        per_device_train_batch_size=1,     # 2 → 1 (VRAM 절약: 배치 크기 축소)
        gradient_accumulation_steps=16,   # 8 → 16 (배치 1로 줄인 만큼 누적 증가, 실질 Batch 16 유지)
        ...
        optim=optimizer,                  # Windows: adamw_torch
        output_dir=OUTPUT_DIR,
        logging_steps=25,
        save_total_limit=1,               # 체크포인트를 최대 1개만 유지
        overwrite_output_dir=True,        # output_dir 재실행 시 덮어쓰기
        dataloader_pin_memory=False,      # VRAM 절약: pin_memory 비활성화
    )
